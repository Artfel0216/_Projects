  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pb-32 font-sans selection:bg-orange-500/30">
      ...
      <header className="sticky top-0 z-50 bg-zinc-950/60 backdrop-blur-xl border-b border-white/5 px-6 py-5 flex items-center justify-between">
        <button 
          onClick={() => router.back()} 
          className="p-2 bg-white/5 rounded-full hover:bg-white/10 transition-colors z-50"
        >
          <ChevronLeft size={20} />
        </button>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md bg-zinc-900/80 backdrop-blur-2xl border border-white/10 h-16 rounded-3xl flex items-center justify-around px-2 z-100 shadow-2xl">
        ...
        <Link href="/TrainingPage" className="w-12 h-12 bg-zinc-800 rounded-2xl flex items-center justify-center -mt-10 border-4 border-zinc-950 shadow-xl hover:bg-orange-600 transition-all active:scale-90">
           <Flame size={24} className="text-white" />
        </Link>

        <Link href="/ProPage" className="flex flex-col items-center gap-1 opacity-40 hover:opacity-100 transition-opacity px-4">
           <Crown size={20} />
           <span className="text-[8px] font-black uppercase tracking-tighter">Pro</span>
        </Link>

        <Link href="/ProfilePage" className="flex flex-col items-center gap-1 opacity-40 hover:opacity-100 transition-opacity px-4">
           <User size={20} />
